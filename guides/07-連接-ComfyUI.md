...
type: 懶人包
version: v0.14
status: 初版（生圖、MiniMax H3 影片與多段短片、Music 3 音樂、Qwen Image Edit 已實測）
tags:
  - 懶人包
  - ComfyUI
  - 生圖
...
**版本** v0.14｜**更新日期** 2026-09-18｜**適用** Claude Code / Codex / OpenCode / Antigravity
...
> 實測下來參考影片**既不控制機位、也不會把畫面內容搬過來**，代價卻是每步 3.6 倍（13.49 → 49.11 秒）。
...
> 欄位、提示詞寫法與實測數據見 `skills/07-comfyui/models/minimax-h3.md`〈Ref2VA〉。
> ⚠️ 網路上「Blender 拉灰模走位與機位、渲成影片當參考」那套是 **Seedance 2.5** 的玩法（它把 clay render 當成專門
> 強化過的參考型別），**不要搬到 H3**——四組對照實測證實機位不會轉移，詳見技能的〈參考影片〉。
...
環境同上，改用 ComfyUI 0.36.0（agent 背景啟動）、comfy-cli 1.20.0。做一支**四段、1056×608、成品約 23 秒**的古裝庭院雙人喜劇短片：
男主角來自使用者的真人照片，女主角與場景由模型生成，對白、音效、配樂都由 H3 一起生。四段各重生 2–9 次，前後共跑約 30 支 H3。
執行時要照做的寫法都收在 `skills/07-comfyui/models/minimax-h3.md`〈Ref2VA〉〈多段影片〉〈FL2VA 首尾幀〉，這裡記原因。

**一致性是怎麼建起來的**（照這個順序）：

1. **角色先有定裝照**。女主角用 Z-Image 生候選、使用者挑一張；男主角用 Qwen Image Edit 把生活照**換裝成定妝照**。
   一開始拿生活照當參考、服裝用文字寫，結果遠景是敞襟粗布短打、特寫卻變立領盤扣——服裝要由圖帶，不能靠字。
2. **以使用者第一個點頭的段落為基準**：抽它一格最完整的廣角、用 Qwen Image Edit 把人移掉，當第三張參考圖（場景版）；
   抽它的音軌當參考聲音。之後各段的門、牆、樹位置與兩人的音色才對得上。
3. **外觀、場景、聲音段落寫成腳本裡的共用字串**，每段逐字相同、只換鏡頭段落。改外觀就改字串、全部重生。

**踩過的坑，以及後來找到的原因**：

- **男主角會禿**。換定妝照、把 `ref_image_size` 改 `max`、在提示詞寫「一頭濃密黑髮」都無效。
  把同一支影片的中景與大特寫並排才看出：**中景都有頭髮，推到頭頂佔滿畫面才退**，而且越往後越嚴重。跟景別綁在一起，
  不是參考圖或用詞的問題。使用者也注意到「太近反而看出臉不大像」——同一件事。之後需要像的角色一律中景。
- **`max` 的代價與效果都跟範本說的不同**：範本註明「最像但慢好幾倍」，實測只慢 5.6%，也沒有更像。
- **參考圖裡的人會被塞進畫面**：提示詞寫女主角在鏡頭外，她照樣站在畫面裡；有一版開頭出現兩個一模一樣的她。
  所以場景版一定要先去人，某段只要一個人時也要拔掉另一張參考圖的線。
- **雙人對話一定會推近**：`locked-off`、`does not move at all` 連試三次都擋不住；同樣的字在單人鏡頭一次就對。
  最後是「場景參考圖＋不准比胸口更近＋起點寫全身」一起下才鎖住。
- **視線撐約 4 秒**：把注視寫成持續狀態後，從 1.4 秒就飄延後到約 4 秒才飄，但無法根除。實務上多生一點、剪在飄走前。
- **不說話比說話難**：只寫「兩人都不說話」，他最後兩秒照樣開口。改成給嘴巴一個任務（抿嘴、吸一口氣但鏡頭在出聲前結束）
  才壓住。
- **5 秒太短會「穿越」**：門還沒開他就已經在院子裡。寫出因果順序（門先開、之後他才出現在門洞、用走的進來）並給到 7 秒才正常。
- **段落接點**：第一段結尾刻意比照第二段開頭的站位，接起來反而像跳一下——兩個畫面「幾乎一樣但差一點」最糟。
  試過把切點往前挪（4 秒 → 3 秒）仍不自然；試過 FL2VA 首尾幀，接縫幾乎無痕，但 FL2VA 不吃參考圖、中段不夠像，使用者選回 Ref2VA。
  最後的解法是讓第一段收在**女主角的單人特寫**，下一段開在雙人廣角——尺寸、主體、角度都不同，就是正常的鏡頭轉換。
- **參考聲音**：拿真人錄音當 `<Audio 1>` 可行（使用者聽過認可），但使用者覺得 H3 原生的聲音比較有張力，
  最後改成以第二段定稿的音軌當參考，統一後面各段的音色。
- **comfy-cli 讀不了新版範本**：0.36.0 的 `video_minimax_h3_i2v` 用 comfy-cli 1.20.0 開啟，`--print-prompt` 與 `slots` 都輸出空字串、
  exit code 卻是 0，不會報錯。改拿已經在跑的 Ref2VA API 圖換節點與主模型，直接 `POST /prompt`。
- **特效一次就成**：先前子彈衝擊波歸納出的「起點、路徑、終點＋前一團未到就發下一團」，用在一團臭氣從甲的嘴飄到乙臉上也一次成功。

**怎麼量**：

- 耗時：`/history/<prompt_id>` 的 `execution_start` 與 `execution_success` 相減（數字表在 `models/minimax-h3.md`）
- 接縫：把前一段最後一格與下一段第一格上下並排比
- 視線、嘴型、進場順序：每 0.5–0.8 秒抽一格排成對照圖；接點前後各抽兩三格
- 清 metadata：`ffprobe -show_entries format_tags` 看標籤，再掃成品位元組確認 `prompt` 與模型檔名出現 0 次
- 聲音一律由使用者試聽（agent 聽不到），包括參考聲音像不像、接點的環境音會不會跳
...
| v0.12 | 2026-09-16 | 步驟八新增〈產出物內嵌完整工作流程〉：實測 PNG 的 `tEXt` 與 MP4 的容器 metadata 都帶完整 API 格式工作流程（提示詞全文、模型檔名、seed），說明讀取方式（`ffprobe -show_entries format_tags`）與發布前另存清除（`ffmpeg -map_metadata -1 -c copy`，不重新編碼、畫質不掉）；同時記錄正面用途——步驟四從舊圖撈回工作流程、確認 H3 加速 LoRA 有沒有生效都靠它。`SKILL.md` 新增步驟 11：使用者要發布時主動提醒一次，同意後另存新檔清除、不覆蓋原檔 |
| v0.13 | 2026-09-16 | 同一台桌機（ComfyUI 0.36.0）下載 Ref2VA 全套約 44 GB，實測 **MiniMax H3 Ref2VA**：只給一張人物照＋文字，就把人放進全新場景與動作（夜間屋頂、後仰躲子彈），半身照一張就保住五官與眼鏡。⚠️ **4 步 turbo LoRA 會毀掉整支**（紅色橫條紋、照抄參考照片的背景與衣服），20 步原始模型一次就正確、每步時間幾乎一樣；深度與機位由 seed 決定（同提示詞 seed 1999 後仰到接近水平＋過肩機位、seed 42 只到 45–55 度）；互相衝突的句子會抵消、借用的動作名詞會帶進它自己的姿勢、特效要寫起點路徑終點。8 秒（192 幀）每步 27.4 秒、總耗時 607–619 秒。步驟九的 Ref2VA 說明改為已實測；`models/minimax-h3.md` 新增〈Ref2VA〉欄位表與提示詞要點；`SKILL.md` 模型筆記索引更新 |
| v0.14 | 2026-09-18 | 同一台桌機（ComfyUI 0.36.0、agent 背景啟動、comfy-cli 1.20.0）實測 **H3 多段短片**：四段、1056×608、成品約 23 秒，前後約 30 支 H3。Ref2VA 用到三張參考圖（定妝照、女主角定裝照、去人的場景版）與參考聲音（`<Audio 1>`），UI 範本沒有的節點在 API 格式補上後 `POST /prompt`。找出：大特寫會禿、`max` 只慢 5.6% 也不更像、參考圖裡的人會被放進畫面、雙人對話推近擋不住、視線約 4 秒衰減、不說話要給嘴巴任務、進場要寫因果順序、段落接點「幾乎一樣」最糟。另測 FL2VA 首尾幀（接縫幾乎無痕但不吃參考圖、較不像）、comfy-cli 讀不了 0.36.0 的 `video_minimax_h3_i2v`（靜默輸出空字串）。新增 **Qwen Image Edit 2511 int8**（真人照換裝成定妝照、畫格去人）。步驟九新增〈4. 多段短片實測〉；`models/minimax-h3.md` 補 Ref2VA 多參考圖與參考聲音、新增〈多段影片〉〈FL2VA 首尾幀〉與 1056×608 耗時表；新增 `models/qwen-image-edit.md`；`SKILL.md` 步驟 8 補 cp950 與 CLI 讀不了範本時的替代路線、模型筆記索引加一列 |
