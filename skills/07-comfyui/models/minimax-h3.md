...
## Ref2VA：把人放進全新場景（參考圖、參考聲音、參考影片都已實測）
...
使用者要「原片原封不動只換人」就明講 H3 做不到，要走逐幀控制的工作流（換臉、控制影片類），不要用本節硬湊。
**參考影片不控制機位也不搬運內容，成本卻是 3.5 倍**，用途很窄（見〈參考影片〉）。
...
- **超過兩張參考圖、要加參考聲音或參考影片**：UI 範本只有 `137`、`139` 兩顆 `LoadImage`，沒有 `LoadAudio`、`LoadVideo`。
  `run --print-prompt` 取得 API 格式後補節點，預檢通過再 `POST /prompt`（`SKILL.md` 步驟 8）：
...
- ⚠️ **`ref_videos.ref_video_0` 的型別是 `IMAGE` 不是 `VIDEO`**（影格序列），所以要兩顆節點轉一手，
  而且畫面與音軌是**分開的插槽**（`ref_video_audios.ref_video_audio_0`，型別 `AUDIO`，不接就是無聲參考）：
  `"301": {"class_type": "LoadVideo", "inputs": {"file": "<檔.mp4>"}}`、
  `"302": {"class_type": "GetVideoComponents", "inputs": {"video": ["301", 0]}}`，
  再 `136.inputs["ref_videos.ref_video_0"] = ["302", 0]`（`GetVideoComponents` 的輸出依序是
  `images`／`audio`／`fps`／`bit_depth`／`color_space`，所以音軌是 `["302", 1]`）。
  `LoadVideo` 的 `file` 是 COMBO，**檔要先在 input 資料夾**；ComfyUI 0.36.0 的 `/object_info` 把選項放在
  `spec[1]["options"]`（不是舊版的 `spec[0]`），用程式核對檔名時要讀對位置。
...
### 參考影片：不控制機位、不搬運內容，成本 3.5 倍（實測）

四組對照（864×480、5 秒、20 步、`beta`、seed 42，提示詞除了機位那句以外逐字相同，每支各自先清快取）。
參考影片是一支既有的生成片：**全程過肩構圖**（前景一大片肩膀）、夜間屋頂、深色衣著男子、同心圓衝擊波特效。
提示詞寫的是**完全無關**的場景：白天明亮廚房、單人女性、切菜。

| 組 | 提示詞有無機位描述 | 參考影片 | 每步 | 總耗時 | 結果構圖 |
|---|---|---|---|---|---|
| A | 有（`locked off on a tripod`、正面中景） | 無 | 13.49 秒 | 315.6 秒 | 正面平視中景（照文字做） |
| B | 有 | 有 | 49.11 秒 | 1075.2 秒 | 正面平視中景（同 A） |
| D | **無**（整句刪掉） | 無 | — | 299.2 秒 | 左 3/4 側面、主體偏左（模型自選） |
| C | **無** | 有 | — | 1068.7 秒 | 左 3/4 側面、主體偏左（**同 D**） |

- **不控制機位。** B 沒被拉走可能是文字壓過去，所以再跑 C／D 把機位那句整段刪掉：**C 與 D 仍然選同一個構圖走向**，
  兩支都沒有出現參考影片那個極強烈的過肩前景。機位的控制手段是**文字**（見〈多段影片〉的「景別與像度」），不是參考影片。
- **不搬運內容。** B、C 都沒有跑出參考影片裡的人、夜景、屋頂或特效。
  這跟參考圖**完全不同**——參考圖只要接了線裡面的人就會入鏡，參考影片不會。所以**不必為參考影片做「去人」處理**。
- **成本是真的貴。** 每步 13.49 → 49.11 秒（3.6 倍），總耗時約 3.5 倍。參考圖幾乎免費，兩者不是同一個量級。
- **有一致的風格偏移**：接了影片的兩組（B 對 A、C 對 D）都出現「鏡頭稍近、光線更暖、對比更強、景深更重」，
  方向一致但屬於調性，不是構圖控制。
- ⚠️ **網路上「Blender 拉灰模走位＋機位、渲成影片丟進去當參考」那套是 Seedance 2.5 的玩法，不要搬到 H3。**
  Seedance 把 clay render／white-model 當成**專門強化過的參考型別**，H3 沒有對應能力：花 3.5 倍時間換不到機位控制。
  要把 3D 的精確性導進 H3，走**渲靜態空景板當參考圖**（`<Picture>`）這條已驗證的路。
- 仍未實測：**帶音軌的參考影片**（`ref_video_audios`）、多段參考影片、以及動作韻律／節奏是否轉移。
