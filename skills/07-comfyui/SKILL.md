...
1. **檢查環境**：`uv --version`；確認 `http://127.0.0.1:8188/system_stats` 有回應。連不上 → 問使用者選哪一種：
   ...
   - **不開 Desktop 視窗，由 agent 在背景以 API 模式啟動**（Windows 的 Desktop 安裝）。路徑依這台讀，不要照抄：
     ...
     **macOS / Linux**
     ...
     ```bash
     cd "<installPath>/ComfyUI" && nohup .venv/bin/python main.py --listen 127.0.0.1 --port 8188 --disable-auto-launch --extra-model-paths-config "<yaml>" --output-directory "<outputDir>" --input-directory "<inputDir>" >/tmp/comfyui.log 2>&1 & echo $!
     ```

     venv 直譯器在 `.venv/bin/python`；`%APPDATA%` 對應 `~/Library/Application Support/Comfy Desktop/`（**未在 mac 實測**）。
...
11. **產出物內嵌完整工作流程**：PNG 在 `tEXt` 的 `prompt`、MP4 與 **mp3** 在容器 metadata 的 `prompt`
    （`ffprobe -v error -show_entries format_tags -of json <檔>`），含提示詞全文、模型檔名與 seed，音樂連**歌詞全文**都在。
    好處是拖回 ComfyUI 就能還原（步驟 5 靠的就是它），**但對外發布等於連提示詞一起送出**。
    使用者說要發布（社群、給別人）時**主動提醒一次**，同意後才清，且**另存新檔、不要覆蓋原檔**：
    影片與 mp3 `ffmpeg -i <輸入> -map_metadata -1 -c copy <輸出>`（不重新編碼）；圖片用 Pillow 重存一份不帶 `info` 的 PNG 或轉 JPG。
...
agent 在背景啟動的 ComfyUI：Windows `Stop-Process -Id $p.Id`，再確認 `Get-NetTCPConnection -LocalPort 8188 -State Listen` 已無結果；macOS / Linux `kill <PID>`，再確認 `lsof -nP -iTCP:8188 -sTCP:LISTEN` 已無結果。
