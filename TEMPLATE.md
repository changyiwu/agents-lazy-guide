...
> **「一行指向 guides/」只是給人看的指引，不是執行時的依賴。** 安裝只複製 `skills/<slug>/`，
> 技能在其他專案載入時讀不到 `guides/`；agent 執行時需要的指令、參數與路徑一律寫在 `SKILL.md` 本文，
> 不可只寫「見 guide」。

長度以 100 行內為目標、**120 行為上限**。背景說明、註冊引導、常見問題留給 `guides/`。

> 超過上限時，先把**取捨說明、踩坑背景、常見問題**搬到 `guides/`；
> **安全規則與 🖐️ 手動標記不可為了省行數刪減**。
>
> **為什麼不是 40–60 行**：這是初版規範，但有分支路線的主題（#05 生圖雙軌、#06 兩條自動部署）
> 光是讓 agent 知道「先問使用者選哪條、各自怎麼做」就放不下，八份技能只剩兩份符合。2026-09-10 放寬。
