...
長度以 120 行內為目標、**150 行為上限**。背景說明、註冊引導、常見問題留給 `guides/`。

> 超過上限時，先把**取捨說明、踩坑背景、常見問題**搬到 `guides/`；
> **安全規則、🖐️ 手動標記、⚠️ 同意點與執行必需的指令不可為了省行數刪減**。
>
> **為什麼不是 40–60 行**：這是初版規範，但有分支路線的主題（#05 生圖雙軌、#06 兩條自動部署）
> 光是讓 agent 知道「先問使用者選哪條、各自怎麼做」就放不下，八份技能只剩兩份符合。
> 2026-09-10 放寬為目標 100／上限 120。
>
> **2026-09-14 再放寬為目標 120／上限 150**：#07 ComfyUI 已剛好卡在 120 行；#06 加入 D1 後，
