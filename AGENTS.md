...
- 2026-09-21：新增 #08 Blender 背景 `bpy` runner；2026-09-22 升級 v0.3，以 Blender Lab 官方 MCP 作即時主線、runner 作可重現備援
...
│   ├── 08-blender/SKILL.md ＋ scripts/、templates/、references/（官方 MCP＋背景 runner）
...
- **#08 Blender 使用已安裝的 Blender 執行檔＋`bpy`**，不以 `pip install bpy` 取代。即時查看、參考圖對形與
  多輪微調走 Blender Lab 官方 MCP；一次性、批次與 CI 走背景 runner。MCP 只綁 `localhost:9876`，安裝／啟用前
  必須說明其會執行 LLM 產生的 Python 並取得同意；不可改成區網或公開介面。
  原始 `.blend` 只當輸入、結果另存專案 `blender/`；兩種模式都停用 autoexec。
